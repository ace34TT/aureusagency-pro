  ...
  return (
    <BlockWrapper id="work" className="">
      ...
      <div className="relative mx-auto container z-10">
        ...
        <div className="mt-8 sm:mt-10 lg:mt-12 grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
          {(projects || []).map((item, index) => (
            <div
              key={item.id || index}
              className="group relative overflow-hidden rounded-[24px] sm:rounded-[28px] border border-[#0F172A]/10 bg-white/80 p-5 sm:p-6 shadow-[0_20px_60px_rgba(15,23,42,0.1)]"
            >
              <div className="relative z-10">
                <div className="absolute -right-10 top-0 h-24 w-24 rounded-full bg-[#9AD5CA]/30 blur-2xl transition group-hover:scale-110" />
                <p className="text-[0.65rem] sm:text-xs uppercase tracking-[0.25em] sm:tracking-[0.3em] text-[#0F172A]/60">
                  {item.name}
                </p>
                <p className="mt-3 sm:mt-4 text-xl sm:text-2xl font-semibold text-[#0F172A]">
                  {item.result}
                </p>
